...
### 2.1 EF_SPNI — Service Provider Name Icon (`0x6FD7`)
...
**Важно**: EF_SPNI никогда не показывается самостоятельно — только в паре с EF_SPN. Если EF_SPN пуст или не читается, иконка игнорируется.
...
UPDATE BINARY 0x6FD7 <PNG bytes>    ← Записать иконку
```

### 2.2 EF_PNNI — PLMN Network Name Icon (`0x6FD8`)
...
EF_PNNI (Transparent, 0x6FD8):
  [PNG-иконка]  ← ОДНА иконка на ВСЕ PNN-записи
```

> [!warning] Важное отличие от SPNI
> EF_PNNI содержит **одну** иконку, которая используется для **всех** записей EF_PNN. В отличие от структуры EF_IMG/EF_IIDF (где может быть много изображений), PNNI — это один плоский файл с одной иконкой. Это ограничение означает, что все PLMN Network Names разделяют одну иконку или не имеют её вовсе.
...
> - **Расположение**: MF (корневой уровень) или внутри DF_CD
...
└── DF_CD (5F01)               ← Config Data (опционально)
    └── DF_GRAPHICS (5F50)     ← Графическая директория
        ├── EF_IMG (5F20)      ← Массив изображений
        ├── EF_IIDF (5F21)     ← Метаданные изображений
        └── EF_IMG_N (5Fxx)    ← Индивидуальные изображения (опционально)
```

> [!note] Примечание
> DF_GRAPHICS может находиться и непосредственно под MF (`3F00/5F50`), и внутри DF_CD (`3F00/5F01/5F50`). Точное расположение определяется реализацией UICC.

#### EF внутри DF_GRAPHICS

**EF_IMG** (`0x5F20`) — основной контейнер изображений:
...
### 2.4 EF_IIDF — Image Instance Data Files (`0x5F21`)
...
**Image Coding Field** (из ETSI TS 102 221):

| Код | Формат | Стандарт |
|---|---|---|
| `0x01` | PNG | ISO/IEC 15948 |
| `0x02` | JPEG/JFIF | ISO/IEC 10918 |
| `0x03` | BMP (main bitmap) | — |
| `0x04` | BMP (alternative bitmap) | — |
| `0x11` | PNG (colour) | — |
| `0x12` | JPEG (colour) | — |
| `0x21` | PNG with transparency | — |
...
> - **Расположение**: DF_CD
> - **Назначение**: Иконки для пунктов STK-меню (SET UP MENU)
...
**Обязательные характеристики** для всех PNG-изображений на UICC:

1. **Структура**: Соответствие ISO/IEC 15948:2004 (PNG specification)
2. **Цветовые режимы**:
...
6. **Максимальный размер**:
   - **Ширина/высота**: 64x64 пикселя (типично), может быть меньше
   - **Размер файла**: Ограничено EEPROM UICC (2-8 КБ типично, до 32 КБ на современных картах)
...
# Записываем SPNI иконку
pySim-shell> update_binary 0x6FD7 <PNG_HEX_DATA>

# Проверяем что записалось
pySim-shell> read_binary 0x6FD7
```
...
| **USIM Service** | Service 12 (SPN) | Service 68 (USAT) |
...
> [!warning]
> Эта таблица основана на тестах сообщества Osmocom и pySim. Реальное поведение может различаться в зависимости от версии прошивки и региона.
