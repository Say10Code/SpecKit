> **📦 АРХИВ** | 2026-06-14 | Предложение реструктуризации (`3GPP/`/`ETSI/`/`GSMA/`) оценено и отложено. Цена (сломанные wikilinks ×130 стр.) > выгоды. Текущая топик-ориентированная структура работает. Отдельные проблемы решаются точечно через Librarian.
